...
func queryPublic(cliCtx context.CLIContext, storeName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/public/%s", storeName, owner), nil)
		if err != nil {
...
func queryPrivate(cliCtx context.CLIContext, storeName string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/private/%s", storeName, owner), nil)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusNotFound, err.Error())
			return
		}
